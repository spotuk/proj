package ua.kpi.getman.InternetShop.bean;
...
public class CartBean {
	...
	public void addCartItem(String strModelNo, String strDescription, String strUnitCost, String strQuantity,
		...
		try {
			...
			if (iQuantity > 0) {
				...
				if (check(cartItem, productId, dblTotalCost, iQuantity)) {
	...
	public boolean check(CartItemBean cartItem, long productId, double dblTotalCost, int iQuantity) {
		boolean flag = false;

		if (!alCartItems.isEmpty()) {
			for (CartItemBean item : getCartItems()) {
				if (item.getProductId().equals(productId)) {
				
					item.setQuantity(item.getQuantity() + iQuantity);
					item.setTotalCost(dblTotalCost * item.getQuantity());
						return true;		}
			}
		}
